...
def convertToTTS_GTF(df_bed, bps):
    ...
    for i in df_bed: 
        if i[5] =="+":
            ...
            if i[2] not in dict_TSS_plus.keys():
        ...
        #If strand is negative 
             ...
             if i[1] not in dict_TSS_neg.keys():
                 genome=i[0]
                 cap="CAPPABLE_SEQ"
                 TNS="TTS"
                 start=int(i[1])
    ...
    df['Nreads']=df.apply(lambda x:equal(x['end'],x['start'], x['strand'],dict_TSS_plus, dict_TSS_neg), axis=1)
