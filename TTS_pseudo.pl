...
my $error_sentence = "USAGE : perl $0\n --in sorted.bam\n --out output.gtf\nOPTIONAL:\n --ratio (minimal ratio of coverage before and after TTS)\nTSS PARAMETERS:\n--combine (bp to merge TSS, DEFAULT 20 bp)\n --filter (minimal absolute coverage of TSS, DEFAULT 5)\n --rpm (minimal reads per million of TSS, DEFAULT 5)" ;
...
sub parse_merged{
    ...
    foreach my $line (<FILE>)
    {
        ...
        my $window = 21;
        my $scanrange = 50;
        if ($strand eq "+" && $total_reads_line >= $FILTER && $rpms >= $RPM){ # for all + strand significant TSS
            my $i = $tsspos + 1;
            while ($i<$tsspos+10000)
            {
                last if($i + $window > $genome_size || $found == 1);
                my $pre = 1;
                my $post = 1;
                foreach my $ele (@depth[$tsspos+1..$tsspos+$window]){
                    $post += $ele;
                }
                foreach my $ele (@depth[$tsspos-$window..$tsspos-1]){
                    $pre += $ele;
                }
                my $ratio = $pre/$post;
                if($depth[$i] == 0 && $depth[$i+1] == 0 && $depth[$i+2] == 0){ # give hit is coverage drops to zero, then goes to next TSS
                    print OUT
                        "$chr\tCAPPABLE_SEQ\tTTS\t$i\t$i\t$tsspos\t$col[2]\t$tsspos\t.\t$total_reads\t.\n";
                    $found = 1;
                    $lasttts = $i;
                    last;
                    }
                if ($ratio >= $RAT && $depth[$i+1] > 3){
                    my $bestpos = $i;
                    my $bestratio = $ratio;
                    my $current = $i;
                    ...
                    print OUT
                        "$chr\tCAPPABLE_SEQ\tTTS\t$bestpos\t$bestpos\t$tsspos\t$col[2]\t$tsspos\t.\t$total_reads\t.\n";
                    $lasttts = $bestpos;
                    $found = 1;
                    $lasttts = $i;
                    last;
        ...
        elsif ($strand eq "-" && $total_reads_line >= $FILTER && $rpms >= $RPM){
            my $i = $tsspos - 1;
            while($i>$tsspos-10000)
            {
                last if($i - $window < 0 || $found == 1);
                my $pre = 1;
                my $post = 1;
                foreach my $ele (@depth[$tsspos+1..$tsspos+$window]){
                    $pre += $ele;
                ...
                my $ratio = $pre/$post;
                if($depth[$i] == 0 && $depth[$i-1] == 0 && $depth[$i-2] == 0){
                    print OUT
                        "$chr\tCAPPABLE_SEQ\tTTS\t$i\t$i\t$tsspos\t$col[2]\t$tsspos\t.\t$total_reads\t.\n";
                    $found = 1;
                    $lasttts = $i;
