...
foreach my $chr (sort keys %$result)
{
	    ...
	    if ($neg >= $CUTOFF)
            {
                ...
                print OUT "$chr\t$generic\t$id\t$start\t$start\t$relative_neg\t-\t.\tnumber_of_read=$abs_neg;total_number_of_read=$count_mapped_read\t$count_mapped_read\n";
            }
	...
	if ($$result{$chr}{$keys}{"+"}{"count"})
	{
		...
		print OUT "$chr\t$generic\t$id\t$start\t$start\t$relative_pos\t+\t.\tnumber_of_read=$abs_pos;total_number_of_read=$count_mapped_read\t$count_mapped_read\n";
	    }
	}
